...
class ViewController: UIViewController {
    ...
    @objc func handleAdd() {
        guard let randomSection = Section.allCases.randomElement()
        else { return }
        let user = User(name: UUID().uuidString, backgroundColor: .random(), section: randomSection)
        addItemsSnapshot(with: [user], section: randomSection)
    }
    
    func createDataSource() -> UserDataSource {
        UserDataSource(tableView: tableView) { tableView, indexPath, user in
...
extension ViewController {
    func setupMockData() -> [User] {
        [User(name: "Pedro", backgroundColor: .systemBlue, section: .main),
         User(name: "José", backgroundColor: .systemRed, section: .vip),
         User(name: "João", backgroundColor: .systemCyan, section: .work)
        ]
    }
}
